...
import { JOURNAL_ENTRIES, WORKS } from "../constants";
  ...
  return (
    <div className="pt-32">
          ...
          <div className="flex justify-between items-end mb-32">
            <div>
              ...
              <h3 className="text-5xl font-serif-jp font-bold">
                {lang === "en"
                  ? "Crystals of Vision & Logic"
                  : "思考と実装の結晶"}
              </h3>
            </div>
            <Link
              to="/works"
              className="text-xs tracking-[0.3em] font-bold uppercase hover:text-blue-600 transition-colors flex items-center gap-3"
            >
              {lang === "en" ? "View All Archive" : "全ての実績を見る"}{" "}
            ...
            {WORKS.slice(0, 2).map((work, idx) => (
              ...
              >
                ...
                <div className="w-full md:w-2/5">
                  ...
                  <h4 className="text-5xl font-serif-display font-bold mb-8 italic">
                    {work.title[lang]}
                  </h4>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-32">
            <div>
              ...
              <Link
                to="/journal"
                className="inline-block px-12 py-6 border border-white text-[10px] tracking-[0.4em] font-bold uppercase hover:bg-white hover:text-black transition-all"
              >
            ...
            <div className="space-y-16">
              {JOURNAL_ENTRIES.map((entry) => (
                <div
                  key={entry.id}
                  className="group border-b border-white/10 pb-16"
                >
                  <p className="text-[10px] font-bold text-gray-500 mb-6 uppercase tracking-[0.2em]">
                    {entry.date}
                  </p>
                  <h4 className="text-3xl font-serif-jp font-bold group-hover:text-blue-500 transition-colors mb-6">
                    {entry.title[lang]}
                  </h4>
                  <p className="text-gray-400 text-sm leading-relaxed mb-8 font-serif-jp">
                    {entry.excerpt[lang]}
                  </p>
                  <Link
                    to={`/journal/${entry.id}`}
                    className="text-[10px] tracking-[0.3em] font-bold uppercase group-hover:text-blue-500"
                  >
      ...
      <section className="py-60 bg-white text-center">
        ...
        <Link
          to="/contact"
          className="text-6xl md:text-[10rem] font-serif-display italic font-bold hover:text-blue-600 transition-all duration-700 block mb-16 leading-none"
        >
