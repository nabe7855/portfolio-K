  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        <Link to="/" className="text-xl font-heading tracking-widest font-bold flex items-center gap-3">
        ...
        <div className="hidden md:flex gap-12 items-center">
          {navLinks.map((link) => (
            <Link
              ...
              className={`text-[10px] tracking-[0.2em] font-bold transition-all hover:text-blue-600 ${
                location.pathname === link.path ? 'text-blue-600' : 'text-gray-400'
              }`}
            >
              {link.name[lang]}
            </Link>
          ...
          >
            {isLoggedIn ? (lang === 'en' ? 'LOGOUT' : 'ログアウト') : (lang === 'en' ? 'LOGIN' : 'ログイン')}
          </button>
        </div>
      </div>
    </nav>
